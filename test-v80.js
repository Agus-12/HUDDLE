/* E2E v80: la vista girada gira hacia el lado correcto según cómo esté
 * acostado el teléfono (su parte de arriba hacia la del teléfono) —
 * más todo v79/v78/v77/v76 */
const fs = require('fs');
...
(async () => {
  ...
  /* v65 */
  ok(srv.includes("const UI_VERSION = 'v80'"), 'servidor en v80');
  ok(js.includes("const APP_VERSION = 'v80'"), 'cliente en v80');
  ...
  ok(!css.includes('height: calc(100dvh - 390px)'), 'v71: pantalla del celular compacta como siempre (sin reproductor alto)');
ok(/\.play-btn\s*\{[^}]*justify-content:\s*center/s.test(css), 'v71: "Toca para empezar" centrado (estilo base, aplica también al celular)');
  ok(css.includes('padding: 24px 8px 2px') && !/\.ctrl-layer\s*\{[^}]*position:\s*fixed/s.test(css), 'v73: barrita DENTRO del reproductor, pegada a su borde de abajo');
  ok(srv.includes("op === 'epPrev' || op === 'epNext'"), 'v74: servidor entiende episodio anterior/siguiente');
  ok(srv.includes('serieCtxFromUrl') && srv.includes('async function datosSerieCuevana'), 'v74: contexto de serie en el servidor');
  ok(idx.includes('id="epNav"') && idx.includes('id="epPrevBtn"') && idx.includes('id="epSearchBtn"'), 'v74: botones de episodio en la sala');
  ok(idx.indexOf('id="btnFs"') < idx.indexOf('id="epNav"') && idx.includes('class="picker-row ep-row hidden" id="epNav"'), 'v75: botones de episodio DEBAJO de "Ampliar", fuera del video');
  ok(css.includes('.ep-row') && !css.includes('.ep-nav'), 'v75: fila de episodios en la barra (sin botones sobre el video)');
  ok(js.includes('autoFsPorOrientacion') && js.includes("(orientation: landscape)"), 'v76: ampliar/regresar según cómo cargues el celular');
  ok(js.includes('fs-girado') && js.includes('sincronizarFsGirado'), 'v77: la pantalla completa siempre se ve horizontal');
  ok(css.includes('.video-shell.fs-girado') && css.includes('rotate(90deg)'), 'v77: CSS de la vista girada');
  ok(js.includes("visibilitychange"), 'v77: al volver de pantalla apagada se acomoda la sala');
  /* v78 */
  ok(js.includes('sala-girada') && js.includes('sincronizarSalaGirada') && js.includes('function vistaGirada'), 'v78: la sala siempre se ve vertical (girada en horizontal)');
  ok(css.includes('body.sala-girada #room'), 'v78: CSS de la sala girada');
  ok(srv.includes("'/api/continue'") && srv.includes('registrarProgreso'), 'v78: servidor guarda el progreso (continuar viendo)');
  ok(idx.includes('id="continueBox"') && idx.includes('Continuar viendo'), 'v78: fila "Continuar viendo" en el inicio');
  ok(js.includes('S.resumeAt') && js.includes('Quedaste en'), 'v78: retomar con la posición guardada');
  ok(js.includes('largo / 2 + (ev.clientY - cY)'), 'v78: la barrita se puede arrastrar con la vista girada');
  /* v79 */
  ok(js.includes('movil-horizontal') && js.includes('sincronizarGiros(); /* v79'), 'v79: clase movil-horizontal desde el arranque');
  ok(css.includes('body.movil-horizontal .landing-card') && css.includes('body.movil-horizontal .peli-loading') && css.includes('body.movil-horizontal .serie-picker'), 'v79: invitación, espera y selector también siempre verticales');
  ok(css.includes('.cont-x'), 'v79: botón de quitar en las tarjetas de Continuar viendo');
  ok(srv.includes("req.method === 'DELETE'") && srv.includes('quedan'), 'v79: API para quitar entradas de Continuar viendo');
  /* v80 */
  ok(js.includes('telefonoTopIzquierda') && js.includes('function dirGiro'), 'v80: se detecta hacia qué lado está acostado el teléfono');
  ok(css.includes('var(--giro-sala)') && css.includes('--giro-sala: -90deg'), 'v80: el sentido del giro depende del lado del teléfono');
  ok(js.includes('__toPage'), 'v80: transposición de toques con sentido');
  ok(js.includes('left: 0, top: 0, width: W, height: H'), 'v80: toques girados en coordenadas de contenido (origen 0,0)');
  ok(js.includes('dirGiro() < 0) ? (largo / 2 - (ev.clientY - cY))'), 'v80: la barrita sigue el sentido del giro');
  ok(!css.includes('.peli-card') && !idx.includes('peli-card'), 'v75: pantalla de espera como siempre (sin tarjeta)');
  ok(js.includes('updateEpNav') && js.includes('epNum'), 'v74: cliente con episodios y espera "Preparando Episodio N"');
  ...
  const nomAni = await pA.evaluate(() => ({ n: document.querySelector('#peliNombre').textContent, e: document.querySelector('#peliEstado').textContent }));
  ok(/Daima/i.test(nomAni.n) && (/Episodio/i.test(nomAni.n) || /Episodio/i.test(nomAni.e)), `espera con nombre: "${nomAni.n}" / "${nomAni.e}"`);
  ...
  if (listoAni) {
    ...
    /* ---------- v74: botones de episodio anterior/siguiente ---------- */
    console.log('— Botones de episodio (v74) —');
    const ser74 = await pA.evaluate(() => S.mirror.serie);
    ok(ser74 && ser74.total >= 10 && ser74.hayNext === true && /episodio 1/i.test(ser74.num || ''), `serie detectada: "${ser74.titulo}" — ${ser74.num} de ${ser74.total}`);
    const nav74 = await pA.evaluate(() => ({
      nav: !document.querySelector('#epNav').classList.contains('hidden'),
      next: !document.querySelector('#epNextBtn').classList.contains('hidden'),
      prev: document.querySelector('#epPrevBtn').classList.contains('hidden'),
      buscar: document.querySelector('#epSearchBtn').classList.contains('hidden'),
    }));
    ok(nav74.nav && nav74.next && nav74.prev && nav74.buscar, 'episodio 1: "Siguiente" visible, sin "Anterior", sin "Buscar"');
    /* los botones viven en la barra de ABAJO (con "Buscar películas" y
     * "Ampliar"): ventana de escritorio para que nada se los tape */
    await pA.setViewport({ width: 1280, height: 800 });
    await sleep(600);
    /* invitado en la misma sala: debe ver EXACTAMENTE lo mismo que el anfitrión */
    const bI = await puppeteer.launch({ headless: 'new', protocolTimeout: 180000, args: ['--no-sandbox', '--disable-dev-shm-usage', '--autoplay-policy=no-user-gesture-required'] });
    const pI = await bI.newPage();
    await pI.goto(BASE + '/#' + codeAni, { waitUntil: 'domcontentloaded', timeout: 60000 });
    await pI.waitForSelector('#inviteHero:not(.hidden)', { timeout: 15000 });
    await pI.type('#userNick', 'Inv' + RUN);
    await pI.click('#btnLogin');
    await pI.waitForFunction(() => !document.querySelector('#room').classList.contains('hidden'), { timeout: 20000 });
    await pI.waitForFunction(() => S.mirror.active && S.mirror.serie && /episodio 1/i.test(S.mirror.serie.num || ''), { timeout: 30000 });
    const permI = await pI.evaluate(() => ({ puede: S.canControl, oculto: document.querySelector('#epNav').classList.contains('hidden') }));
    ok(permI.oculto === !permI.puede, `invitado ${permI.puede ? 'con' : 'sin'} botones de episodio, según los permisos de la sala`);
    /* siguiente → episodio 2 con pantalla de espera para TODOS */
    await pA.click('#epNextBtn');
    await pI.waitForFunction(() => {
      const b = document.querySelector('#peliLoading');
      return b && !b.classList.contains('hidden') && /episodio 2/i.test(document.querySelector('#peliEstado').textContent);
    }, { timeout: 40000 });
    ok(true, 'el INVITADO ve la misma pantalla de espera ("Preparando Episodio 2…")');
    await pI.waitForFunction(() => /episodio-2(?!\d)/.test(S.mirror.url || ''), { timeout: 60000 });
    ok(true, 'el invitado también pasa al episodio 2, igual que el anfitrión');
    await bI.close();
    await pA.waitForFunction(() => /episodio-2(?!\d)/.test(S.mirror.url || ''), { timeout: 40000 });
    await pA.waitForFunction(() => {
      const b = document.querySelector('#peliLoading');
      return b && !b.classList.contains('hidden') && /episodio 2/i.test(document.querySelector('#peliEstado').textContent);
    }, { timeout: 40000 }).catch(() => {});
    const esp74 = await pA.evaluate(() => ({
      visible: !document.querySelector('#peliLoading').classList.contains('hidden'),
      estado: document.querySelector('#peliEstado').textContent,
      nombre: document.querySelector('#peliNombre').textContent,
      poster: !document.querySelector('#peliPoster').classList.contains('hidden'),
    }));
    ok(esp74.visible && /episodio 2/i.test(esp74.estado) && esp74.poster, `pantalla de espera (como siempre): "${esp74.estado}" con carátula`);
    ok(/daima/i.test(esp74.nombre), `espera con el nombre de la serie: "${esp74.nombre}"`);
    let listoEp2 = true;
    await pA.waitForSelector('#playBtn:not(.hidden)', { timeout: 150000 }).catch(() => { listoEp2 = false; });
    ok(listoEp2, 'episodio 2 listo en pausa');
    if (listoEp2) {
      const ser2 = await pA.evaluate(() => S.mirror.serie);
      ok(/episodio 2/i.test(ser2.num || '') && ser2.hayPrev === true, `episodio 2: ${ser2.num} (con anterior disponible)`);
      /* anterior → de regreso al episodio 1 */
      await pA.click('#epPrevBtn');
      await pA.waitForFunction(() => /episodio-1(?!\d)/.test(S.mirror.url || ''), { timeout: 40000 });
      let listoEp1 = true;
      await pA.waitForSelector('#playBtn:not(.hidden)', { timeout: 150000 }).catch(() => { listoEp1 = false; });
      ok(listoEp1, 'episodio 1 de regreso con "Anterior"');
      /* último episodio → "Siguiente" se vuelve "Buscar películas" */
      if (listoEp1) {
        const lastUrl = await pA.evaluate(async () => {
          const r = await fetch('/api/anime/dragon-ball-daima-latino?site=latanime');
          const d = await r.json();
          return d.episodios[d.episodios.length - 1].url;
        });
        await pA.evaluate((u) => { $('#mirrorUrl').value = u; startMirrorFromPicker(); }, lastUrl);
        let listoLast = true;
        await pA.waitForSelector('#playBtn:not(.hidden)', { timeout: 150000 }).catch(() => { listoLast = false; });
        ok(listoLast, 'último episodio listo en pausa');
        if (listoLast) {
          const last74 = await pA.evaluate(() => ({
            s: S.mirror.serie,
            nextOculto: document.querySelector('#epNextBtn').classList.contains('hidden'),
            buscarVisible: !document.querySelector('#epSearchBtn').classList.contains('hidden'),
          }));
          ok(last74.s && last74.s.hayNext === false && last74.nextOculto && last74.buscarVisible, 'último episodio: "Siguiente" se vuelve "Buscar películas"');
        }
      }
    }
    ...
    await sleep(2500);
    await pA.setViewport({ width: 800, height: 600 });
  }
  ...
  if (listoPeli) {
    ...
    /* ---------- v78: continuar viendo — la peli a medias queda guardada ---------- */
    console.log('— Continuar viendo (v78) —');
    const tPeli = await pA.evaluate(() => (S.mirrorTime ? Math.floor(S.mirrorTime.t) : 0));
    const listaA = await pA.evaluate(async () => {
      const r = await fetch('/api/continue?name=' + encodeURIComponent(S.profile.name) + '&tok=' + encodeURIComponent(S.profile.token));
      return await r.json();
    });
    const entA = listaA.items && listaA.items.find((e) => /mayday/i.test(e.title || '') || /mayday/i.test(e.url || ''));
    ok(!!entA, `servidor guardó la peli a medias ("${entA ? entA.title : '—'}")`);
    ok(entA && entA.t >= Math.max(0, tPeli - 10) && entA.d > 60, `posición guardada: ${entA ? entA.t + 's de ' + entA.d + 's' : '—'}`);
    await pA.evaluate(() => { try { sendAction({ type: 'mirror', op: 'stop' }); } catch {} });
    await sleep(2000);
    /* la fila "Continuar viendo" aparece en el inicio */
    await pA.goto(BASE + '/', { waitUntil: 'domcontentloaded', timeout: 30000 });
    let listoCont = true;
    await pA.waitForSelector('#continueBox:not(.hidden)', { timeout: 15000 }).catch(() => { listoCont = false; });
    ok(listoCont, 'inicio: fila "Continuar viendo" visible');
    if (listoCont) {
      const cardMay = await pA.evaluate(() => {
        const cards = [...document.querySelectorAll('#continueRow .cont-card')];
        const c = cards.find((x) => /mayday/i.test(x.querySelector('.sr-nombre').textContent || ''));
        if (!c) return null;
        return {
          titulo: c.querySelector('.sr-nombre').textContent,
          txt: c.querySelector('.cont-tiempo').textContent,
          pct: (c.querySelector('.cont-barra i') || {}).style ? c.querySelector('.cont-barra i').style.width : '',
        };
      });
      ok(!!cardMay, `tarjeta de Mayday en la fila${cardMay ? ' — ' + cardMay.txt : ''}`);
      ok(cardMay && /Quedaste en/.test(cardMay.txt) && parseInt(cardMay.pct) > 0, `tarjeta con avance (${cardMay ? cardMay.pct : '—'})`);
      /* retomar: un toque reabre la peli DONDE se quedó */
      if (entA && cardMay) {
        await pA.evaluate(() => {
          const c = [...document.querySelectorAll('#continueRow .cont-card')].find((x) => /mayday/i.test(x.querySelector('.sr-nombre').textContent || ''));
          if (c) c.click();
        });
        await pA.waitForFunction(() => location.hash.match(/^#[A-Z0-9]{4,8}$/), { timeout: 20000 });
        let listoRet = true;
        await pA.waitForSelector('#playBtn:not(.hidden)', { timeout: 150000 }).catch(() => { listoRet = false; });
        ok(listoRet, 'retomar: la peli quedó lista (botón de play)');
        if (listoRet) {
          await pA.waitForFunction((tg) => S.mirrorTime && S.mirrorTime.t >= tg - 6, { timeout: 40000 }, entA.t).catch(() => {});
          const tRet = await pA.evaluate(() => (S.mirrorTime ? Math.floor(S.mirrorTime.t) : -1));
          ok(tRet >= entA.t - 6, `retomar: siguió donde se quedó (${tRet}s, guardado ${entA.t}s)`);
        }
        /* v79: quitar la entrada con la X de la tarjeta — primero
         * paramos el espejo y volvemos al inicio */
        await pA.evaluate(() => { try { sendAction({ type: 'mirror', op: 'stop' }); } catch {} });
        await sleep(2000);
        await pA.goto(BASE + '/', { waitUntil: 'domcontentloaded', timeout: 30000 });
        let listoX = true;
        await pA.waitForSelector('#continueBox:not(.hidden)', { timeout: 15000 }).catch(() => { listoX = false; });
        if (listoX) {
          const clicX = await pA.evaluate(() => {
            const c = [...document.querySelectorAll('#continueRow .cont-card')].find((x) => /mayday/i.test(x.querySelector('.sr-nombre').textContent || ''));
            if (!c) return false;
            c.querySelector('.cont-x').click();
            return true;
          });
          await sleep(800);
          const trasX = await pA.evaluate(async () => {
            const hayMay = [...document.querySelectorAll('#continueRow .cont-card')].some((x) => /mayday/i.test(x.querySelector('.sr-nombre').textContent || ''));
            const r = await fetch('/api/continue?name=' + encodeURIComponent(S.profile.name) + '&tok=' + encodeURIComponent(S.profile.token));
            const d = await r.json();
            return { hayMay, apiMay: (d.items || []).some((e) => /mayday/i.test(e.title || '') || /mayday/i.test(e.url || '')) };
          });
          ok(clicX, 'v79: la tarjeta trae su botón de quitar (X)');
          ok(!trasX.hayMay && !trasX.apiMay, 'v79: la X quita la entrada (de la tarjeta y del servidor)');
        } else {
          ok(false, 'v79: fila Continuar viendo para probar la X');
        }
      }
    }
  }
  ...
  /* v74: las series de Cuevana también traen su contexto */
  const serPk = await pA.evaluate(() => S.mirror.serie);
  ok(serPk && /1x1/.test(serPk.num || '') && serPk.hayNext === true, `serie de Cuevana detectada: "${serPk.titulo}" ${serPk.num} (con siguiente)`);
  ...
  /* ---------- v78: lo que ven JUNTOS queda en el continuar de AMBOS ---------- */
  console.log('— Continuar viendo compartido (v78) —');
  await pA.evaluate(() => { try { sendAction({ type: 'mirror', op: 'play' }); } catch {} });
  await pB.waitForFunction(() => S.mirrorTime && S.mirrorTime.t >= 8, { timeout: 60000 }).catch(() => {});
  const tSer = await pB.evaluate(() => (S.mirrorTime ? Math.floor(S.mirrorTime.t) : -1));
  ok(tSer >= 8, `episodio corriendo para los dos (${tSer}s)`);
  const listaB = await pB.evaluate(async () => {
    const r = await fetch('/api/continue?name=' + encodeURIComponent(S.profile.name) + '&tok=' + encodeURIComponent(S.profile.token));
    return await r.json();
  });
  const entB = listaB.items && listaB.items.find((e) => /peaky/i.test(e.title || '') || /peaky/i.test(e.url || ''));
  ok(!!entB, `el INVITADO también tiene la entrada ("${entB ? entB.title : '—'}" ${entB ? entB.ep || '' : ''})`);
  ok(entB && /1x1/.test(entB.ep || ''), 'entrada del invitado con su episodio (1x1)');
  ...
  const navB = await pB.evaluate(() => ({ puede: S.canControl, oculto: document.querySelector('#epNav').classList.contains('hidden') }));
  ok(navB.oculto === !navB.puede, `invitado ${navB.puede ? 'con' : 'sin'} botones de episodio, según sus permisos`);
  ok(errsB.length === 0, `B sin errores JS${errsB.length ? ' → ' + errsB[0] : ''}`);
  await bB.close();
  await sleep(500);

  /* ---------- v68: en CELULAR el botón de play va ABAJO del video ---------- */
  console.log('— Celular (v73) —');
  ...
  /* v80: simulamos un teléfono acostado con la parte de ARRIBA a la
   * izquierda (el agarre común) — la vista debe girar al otro lado */
  await pM.evaluate(() => {
  ...
  await sleep(900);
  const fsInv = await pM.evaluate(() => fsActive());
  ok(fsInv === false, 'en la pantalla de invitación el volteo no amplía nada');
  /* v79: la tarjetita de invitación se ve VERTICAL (girada) en horizontal */
  const invV = await pM.evaluate(() => {
    const cs = getComputedStyle(document.querySelector('.landing-card'));
    return {
      mh: document.body.classList.contains('movil-horizontal'),
      w: Math.round(parseFloat(cs.width)), h: Math.round(parseFloat(cs.height)),
      tr: cs.transform !== 'none',
      ccw: /matrix\(0(?:px)?, -1/.test(String(cs.transform)),
    };
  });
  ok(invV.mh && invV.tr && invV.h > invV.w, `v79: invitación girada — se ve vertical (${invV.w}x${invV.h})`);
  ok(invV.ccw, 'v80: con el teléfono acostado al lado común, gira hacia el lado correcto (no al revés)');
  await pM.setViewport({ width: 390, height: 844, isMobile: true, hasTouch: true });
  await sleep(900);
  const invP = await pM.evaluate(() => !document.body.classList.contains('movil-horizontal') && getComputedStyle(document.querySelector('.landing-card')).transform === 'none');
  ok(invP, 'v79: de vuelta en vertical la invitación se endereza');
  ...
  await sleep(800);
  /* ---------- v76: voltear el celular → amplía solo; levantarlo → sala ---------- */
  console.log('— Voltear el celular (v76) —');
  const fs0 = await pM.evaluate(() => fsActive());
  ok(fs0 === false, 'en vertical: modo sala (sin ampliar)');
  await pM.setViewport({ width: 844, height: 390, isMobile: true, hasTouch: true });
  await sleep(1200);
  const fs1 = await pM.evaluate(() => fsActive());
  ok(fs1 === true, 'al VOLTEARLO (horizontal) se amplía solo');
  await pM.setViewport({ width: 390, height: 844, isMobile: true, hasTouch: true });
  await sleep(1200);
  const fs2 = await pM.evaluate(() => fsActive());
  ok(fs2 === false, 'al LEVANTARLO (vertical) regresa al modo sala');
  await sleep(800);
  /* ---------- v77: ampliar en vertical → vista HORIZONTAL girada ---------- */
  console.log('— Ampliar en vertical (v77) —');
  await pM.evaluate(() => { try { toggleFullscreen(); } catch {} });
  await sleep(900);
  const g1 = await pM.evaluate(() => ({ fs: fsActive(), girado: document.querySelector('#videoShell').classList.contains('fs-girado') }));
  ok(g1.fs && g1.girado, 'ampliar en vertical: la vista se pone HORIZONTAL (girada)');
  /* v80: en pantalla completa REAL el navegador ignora los transforms
   * (regla propia de Chrome) — la vista girada vive en la pantalla
   * completa SIMULADA (la de iPhone): verificamos esa ruta */
  await pM.evaluate(() => { try { exitFullscreen(); } catch {} });
  await sleep(500);
  await pM.evaluate(() => { try { enterPseudoFs(); } catch {} });
  await sleep(500);
  const matPS = await pM.evaluate(() => String(getComputedStyle(document.querySelector('#videoShell')).transform).slice(0, 22));
  ok(/matrix\(0(?:px)?, 1/.test(matPS), `v80: la ampliada simulada (iPhone) gira igual que siempre: ${matPS}`);
  await pM.evaluate(() => { try { exitFullscreen(); } catch {} });
  await sleep(500);
  await pM.setViewport({ width: 844, height: 390, isMobile: true, hasTouch: true });
  await sleep(1200);
  const g2 = await pM.evaluate(() => ({ fs: fsActive(), girado: document.querySelector('#videoShell').classList.contains('fs-girado') }));
  ok(g2.fs && !g2.girado, 'al voltear el celular la vista se endereza (sigue ampliada)');
  await pM.setViewport({ width: 390, height: 844, isMobile: true, hasTouch: true });
  await sleep(1200);
  const g3 = await pM.evaluate(() => fsActive());
  ok(g3 === false, 'de vuelta en vertical regresa al modo sala');
  /* volver de pantalla apagada con la sala ampliada → modo sala */
  await pM.evaluate(() => { try { toggleFullscreen(); } catch {} });
  await sleep(900);
  const g4a = await pM.evaluate(() => fsActive());
  await pM.evaluate(() => {
    Object.defineProperty(document, 'visibilityState', { get: () => 'visible', configurable: true });
    document.dispatchEvent(new Event('visibilitychange'));
  });
  await sleep(1200);
  const g4 = await pM.evaluate(() => fsActive());
  ok(g4a && !g4, 'al VOLVER de pantalla apagada (vertical) regresa al modo sala');
  /* volver de pantalla apagada en horizontal → ampliado */
  await pM.setViewport({ width: 844, height: 390, isMobile: true, hasTouch: true });
  await sleep(1200);
  await pM.evaluate(() => { try { exitFullscreen(); } catch {} });
  await sleep(600);
  ...
  await sleep(1200);
  const g5 = await pM.evaluate(() => fsActive());
  ok(g5 === true, 'al volver de pantalla apagada en HORIZONTAL se amplía');
  /* ---------- v78: la sala SIEMPRE se ve vertical ---------- */
  console.log('— Sala siempre vertical (v78) —');
  /* X en horizontal → sala girada (se ve vertical, voltea el teléfono) */
  await pM.evaluate(() => { try { exitFullscreen(); } catch {} });
  await sleep(1100);
  const sg1 = await pM.evaluate(() => {
    const room = document.querySelector('#room');
    const cs = getComputedStyle(room);
    return {
      fs: fsActive(),
      girada: document.body.classList.contains('sala-girada'),
      w: Math.round(parseFloat(cs.width)), h: Math.round(parseFloat(cs.height)),
      rectW: Math.round(room.getBoundingClientRect().width), rectH: Math.round(room.getBoundingClientRect().height),
    };
  });
  ok(!sg1.fs && sg1.girada, 'X en horizontal: la sala se muestra GIRADA (vertical)');
  ok(sg1.h > sg1.w && sg1.h >= 700, `sala acomodada en vertical (${sg1.w}x${sg1.h}) dentro del celular horizontal`);
  ok(sg1.rectW > sg1.rectH, `la sala girada llena la pantalla horizontal (${sg1.rectW}x${sg1.rectH})`);
  /* v80: el sentido del giro + los toques caen donde se ven.
   * (con el espejo en pausa puede que no haya llegado ningún frame al
   * celular → forzamos uno con un seek pequeño desde el anfitrión) */
  await pA.evaluate(() => { try { sendAction({ type: 'mirror', op: 'seekTo', time: Math.max(1, Math.floor((S.mirrorTime ? S.mirrorTime.t : 10) - 2)) }); } catch {} });
  await pM.waitForFunction(() => S.mirror.w > 0 && S.mirror.h > 0, { timeout: 20000 }).catch(() => {});
  const dir1 = await pM.evaluate(() => {
    const m = String(getComputedStyle(document.querySelector('#room')).transform);
    const rc = document.querySelector('#videoShell').getBoundingClientRect();
    const p = window.__toPage ? window.__toPage(rc.left + rc.width / 2 - 60, rc.top + rc.height / 2) : null;
    return { ccw: /matrix\(0(?:px)?, -1/.test(m), y: p ? p.y : null, h: S.mirror.h };
  });
  ok(dir1.ccw, 'v80: sala girada hacia el lado correcto (arriba de la vista = arriba del teléfono)');
  ok(dir1.y !== null && dir1.y < dir1.h / 2, `v80: toque a la IZQUIERDA del centro cae en la mitad de ARRIBA de la página (y=${dir1.y})`);
  /* y acostado al OTRO lado, gira al lado contrario */
  await pM.evaluate(() => {
    Object.defineProperty(screen, 'orientation', { value: { angle: 270, type: 'landscape-secondary' }, configurable: true });
    sincronizarGiros();
  });
  await sleep(300);
  const dir2 = await pM.evaluate(() => {
    const m = String(getComputedStyle(document.querySelector('#room')).transform);
    const rc = document.querySelector('#videoShell').getBoundingClientRect();
    const p = window.__toPage(rc.left + rc.width / 2 - 60, rc.top + rc.height / 2);
    return { cw: /matrix\(0(?:px)?, 1/.test(m), y: p ? p.y : null, h: S.mirror.h };
  });
  ok(dir2.cw, 'v80: acostado al otro lado, la sala gira al lado contrario');
  ok(dir2.y !== null && dir2.y > dir2.h / 2, `v80: y el mismo toque ahora cae en la mitad de ABAJO (y=${dir2.y})`);
  await pM.evaluate(() => {
    Object.defineProperty(screen, 'orientation', { value: { angle: 90, type: 'landscape-primary' }, configurable: true });
    sincronizarGiros();
  });
  await sleep(300);
  /* los toques caen donde se ven: "Ampliar" dentro de la sala girada */
  await pM.evaluate(() => { try { tocarPantallaCine(); } catch {} });
  await sleep(500);
  const barV = await pM.evaluate(() => {
    const r = document.querySelector('#seekBar').getBoundingClientRect();
    return { w: Math.round(r.width), h: Math.round(r.height) };
  });
  ok(barV.h > barV.w, `barrita girada con la sala (se recorre con el dedo arriba/abajo: ${barV.w}x${barV.h})`);
  /* v79: la pantalla de espera y el selector de episodios, también girados */
  const espV = await pM.evaluate(() => {
    S.mirrorInfo = { title: 'Prueba', img: '', url: '', sub: 'x' };
    mostrarPeliLoading();
    const cs = getComputedStyle(document.querySelector('#peliLoading'));
    return { w: Math.round(parseFloat(cs.width)), h: Math.round(parseFloat(cs.height)), tr: cs.transform !== 'none' };
  });
  ok(espV.tr && espV.h > espV.w, `v79: pantalla de espera "Preparando…" girada (${espV.w}x${espV.h})`);
  await pM.evaluate(() => { try { ocultarPeliLoading(); } catch {} });
  await sleep(300);
  await pM.evaluate(() => { abrirSeriePicker({ url: 'https://latanime.org/anime/dragon-ball-daima-latino', title: 'Dragon Ball Daima Latino', img: '' }, false, true); });
  await pM.waitForSelector('#seriePicker:not(.hidden)', { timeout: 15000 });
  await pM.waitForFunction(() => document.querySelectorAll('#spEpisodios .sp-ep').length > 0, { timeout: 30000 });
  const selV = await pM.evaluate(() => {
    const cs = getComputedStyle(document.querySelector('#seriePicker'));
    const cuerpo = document.querySelector('.sp-cuerpo');
    cuerpo.scrollTop = 60;
    const st = cuerpo.scrollTop;
    return { w: Math.round(parseFloat(cs.width)), h: Math.round(parseFloat(cs.height)), tr: cs.transform !== 'none', scroll: st };
  });
  ok(selV.tr && selV.h > selV.w, `v79: selector de episodios girado (${selV.w}x${selV.h})`);
  ok(selV.scroll > 0, `v79: la lista de episodios se desliza girada (scrollTop ${selV.scroll})`);
  await pM.click('#spClose');
  await sleep(600);
  const selC = await pM.evaluate(() => document.querySelector('#seriePicker').classList.contains('hidden'));
  ok(selC, 'v79: los toques funcionan en el selector girado (se cierra con la X)');
  await pM.evaluate(() => { try { mostrarCtrls5s(); } catch {} }); /* re-mostrar controles SIN alternar play/pausa */
  await sleep(400);
  await pM.click('#btnFs');
  await sleep(1100);
  const sg2 = await pM.evaluate(() => ({
    fs: fsActive(),
    girada: document.body.classList.contains('sala-girada'),
    videoGirado: document.querySelector('#videoShell').classList.contains('fs-girado'),
  }));
  ok(sg2.fs && !sg2.girada && !sg2.videoGirado, 'tocar "Ampliar" en la sala girada → pantalla completa horizontal normal');
  /* X otra vez → sala girada de nuevo; al ponerlo vertical se endereza */
  await pM.evaluate(() => { try { exitFullscreen(); } catch {} });
  await sleep(1000);
  const sg3 = await pM.evaluate(() => document.body.classList.contains('sala-girada'));
  ok(sg3 === true, 'X otra vez → sala girada de nuevo');
  await pM.setViewport({ width: 390, height: 844, isMobile: true, hasTouch: true });
  await sleep(1300);
  const sg4 = await pM.evaluate(() => ({ fs: fsActive(), girada: document.body.classList.contains('sala-girada') }));
  ok(!sg4.fs && !sg4.girada, 'al ponerlo vertical la sala se endereza sola (normal)');
  /* dejamos el celular en vertical y en modo sala para las medidas */
  await pM.setViewport({ width: 390, height: 844, isMobile: true, hasTouch: true });
  await sleep(900);
  await pM.evaluate(() => { try { exitFullscreen(); } catch {} });
  await sleep(900);
  const posM = await pM.evaluate(() => {
    ...
    return {
      ...
      shellT: S.top, shellB: S.bottom, shellH: S.height,
      barT: W.top, barB: W.bottom,
      navTop: N.top,
      textoDebajo: T.top >= C.bottom,
    };
  });
  ok(posM.shellH < 300, `v71: pantalla compacta como siempre (${Math.round(posM.shellH)}px, sin expandir)`);
  ok(Math.abs(posM.circX - 195) <= 20, `v71: círculo de play centrado a lo ancho (${Math.round(posM.circX)} de 195)`);
  const centroShellM = (posM.shellT + posM.shellB) / 2;
  ok(Math.abs(posM.circY - centroShellM) <= 40, `v71: círculo de play EN MEDIO del reproductor (${Math.round(posM.circY)} vs centro ${Math.round(centroShellM)})`);
  ok(posM.textoDebajo, 'v71: "Toca para empezar" DEBAJO del círculo');
  ok(posM.barB <= posM.shellB + 1, `v73: barrita DENTRO del reproductor (${Math.round(posM.barB)} ≤ ${Math.round(posM.shellB)})`);
  ok(posM.shellB - posM.barB <= 12, `v73: barrita pegada al borde de abajo del reproductor (${Math.round(posM.shellB - posM.barB)}px de aire)`);
  ok(posM.barT > posM.shellT + posM.shellH / 2, `v73: barrita en la mitad de abajo del reproductor (${Math.round(posM.barT)})`);
  ok(posM.navTop < posM.barT, 'celular: botones 10/30/lupa arriba de la barrita');
  const rM74 = await pM.evaluate(() => ({ puede: S.canControl, oculto: document.querySelector('#epNav').classList.contains('hidden') }));
  ok(rM74.oculto === !rM74.puede, `celular: botones de episodio ${rM74.puede ? 'visibles' : 'ocultos'} según permisos`);
  /* v78: sin nada en el espejo la sala TAMBIÉN se ve vertical en horizontal */
  await pA.evaluate(() => { try { sendAction({ type: 'mirror', op: 'stop' }); } catch {} });
  await sleep(1500);
  await pM.setViewport({ width: 844, height: 390, isMobile: true, hasTouch: true });
  await sleep(1300);
  const sg5 = await pM.evaluate(() => ({ fs: fsActive(), girada: document.body.classList.contains('sala-girada') }));
  ok(!sg5.fs && sg5.girada, 'sala girada también sin nada en el espejo (siempre vertical)');
