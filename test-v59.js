/* E2E v59: invitación + volver + series (cine-calidad) + arranque automático
 * + pantalla de espera con carátula + pantalla completa */
...
(async () => {
  ...
  /* v59: pantalla de espera con carátula */
  await pA.waitForSelector('#peliLoading:not(.hidden)', { timeout: 20000 });
  ...
  ok(!!peli.nombre, `pantalla de espera: "${peli.nombre}"`);
  ok(/serie/i.test(peli.estado), `texto serie: "${peli.estado}" / "${peli.sub}"`);
  ok(/w342|image\.tmdb/.test(peli.poster), `carátula: ${peli.poster}`);
  /* tocar la pantalla de espera la quita (serie = manual) */
  await pA.click('#peliLoading');
  await sleep(600);
  const plOculto = await pA.$eval('#peliLoading', (n) => n.classList.contains('hidden'));
  ok(plOculto, 'tocar la espera la cierra');
  ...
  await pA.waitForFunction(() => document.querySelector('#peliLoading').classList.contains('hidden'), { timeout: 30000 });
