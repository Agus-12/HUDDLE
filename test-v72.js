/* E2E v72: en celular (sin ampliar) la barrita de adelantar va FIJA
 * abajo, casi al borde de la pantalla, justo encima de la caja del
 * chat — más todo v71/v70/v69 (play centrado, búsqueda Cuevana+Latanime) */
const fs = require('fs');
...
(async () => {
  ...
  /* v65 */
  ok(srv.includes("const UI_VERSION = 'v72'"), 'servidor en v72');
  ok(js.includes("const APP_VERSION = 'v72'"), 'cliente en v72');
...
ok(/\.play-btn\s*\{[^}]*justify-content:\s*center/s.test(css), 'v71: "Toca para empezar" centrado (estilo base, aplica también al celular)');
  ok(css.includes('.video-shell:not(:fullscreen):not(:-webkit-full-screen):not(.pseudo-fs) .ctrl-layer') && /\.ctrl-layer\s*\{[^}]*position:\s*fixed/s.test(css), 'v72: barrita fija abajo en el celular');
  ok(css.includes('bottom: calc(70px + env(safe-area-inset-bottom, 0px))'), 'v72: casi al borde, encima de la caja del chat');
  ...
  const posM = await pM.evaluate(() => {
    ...
    const N = document.querySelector('#mirrorNav').getBoundingClientRect();
    const W = document.querySelector('#seekWrap').getBoundingClientRect();
    const F = document.querySelector('.chat-form').getBoundingClientRect();
    return {
      circX: (C.left + C.right) / 2, circY: (C.top + C.bottom) / 2,
      shellT: S.top, shellB: S.bottom, shellH: S.height,
      barT: W.top, barB: W.bottom, altoPantalla: innerHeight,
      chatT: F.top, navTop: N.top,
  ...
  ok(posM.barT > posM.shellB, `v72: barrita DEBAJO del video (${Math.round(posM.barT)} vs fin del video ${Math.round(posM.shellB)})`);
  ok(posM.barB > posM.altoPantalla - 120, `v72: barrita casi al borde de la pantalla (${Math.round(posM.barB)} de ${posM.altoPantalla})`);
  ok(posM.chatT - posM.barB >= 0, `v72: sin chocar con la caja del chat (barra ${Math.round(posM.barB)} ≤ chat ${Math.round(posM.chatT)})`);
