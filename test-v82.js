/* E2E v82: modo individual más cómodo — la barra de arriba también se
 * esconde tras 5s (pelis sin barras), la reconexión por el proxy te
 * devuelve a tu minuto (no al inicio), el celular acostado se amplía
 * solo, y aviso "Reanudando en mm:ss" + todo v81. */
const fs = require('fs');
...
(async () => {
  ...
  ok(srv.includes("const UI_VERSION = 'v82'"), 'servidor en v82');
  ok(js.includes("const APP_VERSION = 'v82'"), 'cliente en v82');
  ok(idx.includes('id="verBadge">v82'), 'badge v82');
  ok(idx.includes('/app.js?v=82') && idx.includes('/style.css?v=82'), 'cache-busters v82');
  /* hls.js vendored */
  ...
  ok(js.includes('const reSeek') && js.includes('const reT = SOLO.seekHecho ? SOLO.lastT : 0') && js.includes('reT < video.duration - 10'), 'JS: al reconectar vuelve a su minuto (congelado antes del remount)');
  ...
  const tPre = await pA.evaluate(() => Math.floor(document.querySelector('#soloVideo').currentTime));
  const recon = await pA.evaluate((tObjetivo) => {
  ...
  for (let i = 0; i < 40; i++) {
    await sleep(2000);
    const st = await pA.evaluate(() => { const v = document.querySelector('#soloVideo'); return { rs: v.readyState, t: v.currentTime, pausa: v.paused }; });
    if (st.rs >= 2 && !st.pausa && Math.abs(st.t - Math.max(60, tPre)) < 20) { volvio = true; tVolvio = st.t; break; }
  }
  ok(volvio, `reconexión por el proxy vuelve al minuto (iba en ${tPre}s → ${tVolvio.toFixed(0)}s)`);
