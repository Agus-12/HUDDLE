/* E2E v68: sin AnimeFLV ni GoPelis, chip del sitio junto a "En vivo",
 * botón "Buscar películas" en la sala y play ABAJO en celular — más todo
 * lo de v67 (animes del momento, proxy propio de imágenes) y v66/v65/v64 */
const fs = require('fs');
...
(async () => {
  ...
  /* v65 */
  ok(srv.includes("const UI_VERSION = 'v68'"), 'servidor en v68');
  ok(js.includes("const APP_VERSION = 'v68'"), 'cliente en v68');
  ...
  const bB = await puppeteer.launch({ headless: 'new', args: ['--no-sandbox', '--disable-dev-shm-usage', '--autoplay-policy=no-user-gesture-required'] });
  ...
  const bM = await puppeteer.launch({ headless: 'new', args: ['--no-sandbox', '--disable-dev-shm-usage', '--autoplay-policy=no-user-gesture-required'] });
