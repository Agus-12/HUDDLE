...
- **Identificar QUÉ episodio es cada carpeta:** `identificar-movie.js ~/movie-cosecha.json` consulta y cachea las fichas web, deduplica el catálogo histórico repetido, suma las 83 fichas actuales de la vitrina Telenovela pública y compara `vod_duration` con `EXTINF` (±12 s). Deja `~/movie-episodios-cache.json`, `~/movie-coincidencias.json` y `~/movie-coincidencias.txt`. La prueba amplificada indexó 4,866 episodios y encontró candidatos para 6/6; para la ruta de 49:09 apareció `Camino a Arcadia` T1 Ep. 3 a +0.360 s. **No es una asignación definitiva:** muchas novelas comparten exactamente estas duraciones (en varias rutas hay 150–400 candidatos); confirmar qué títulos reprodujo el amigo y/o recuperar el orden temporal de las peticiones antes de guardar el mapa.
- **Orden de la captura sin recapturar:** las dos primeras versiones de `ordenar-pcap-movie.js` basadas en filtros BPF de `tcpdump` devolvieron 0 en este PCAP aunque la extracción original sí vio las rutas; no concluir que falten datos. La versión actual lee **PCAP/PCAPNG binario directamente** en bloques, busca el patrón exacto `/vod/1/.../index5.m3u8` dentro de cada paquete y conserva su timestamp, sin imprimir/guardar segmentos. Está probada con fixtures PCAP y PCAPNG. Tras actualizar, volver a correr SOLO este análisis: no requiere ni modifica el PCAP ni pide otra captura. Deja `~/movie-pcap-orden.txt/.json` con manifests en orden y peticiones HTTP de portada/ficha que estén en claro, sin query/tokens/cookies.
...
- `ordenar-pcap-movie.js` — escanea directamente PCAP/PCAPNG YA recibido, por bloques y sin depender de `tcpdump`, para generar orden temporal de rutas M3U8 + posibles requests HTTP de portada/ficha, con queries saneadas; no descarga video ni altera el PCAP. Sus reportes locales quedan ignorados por git.
...
3. La captura #1 YA está en Oracle (`~/captura-movie.pcap`, 636.6 MiB) y ya fue cosechada: 39 rutas → 6 M3U8 válidos (4:22:41.824) + 33 HTTP 403. No repetir captura ni borrar esos archivos. Correr `node identificar-movie.js ~/movie-cosecha.json` y `node ordenar-pcap-movie.js ~/captura-movie.pcap ~/movie-cosecha.json`; leer `~/movie-coincidencias.txt` + `~/movie-pcap-orden.txt` y confirmar con el amigo los títulos reproducidos antes de guardar el mapa. Para nuevas capturas: extraer rutas `/vod/1/.../index5.m3u8` (§2) y repetir el reporte. No subir PCAP ni reportes crudos al repo público.
