...
- **Orden de la captura sin recapturar:** `ordenar-pcap-movie.js ~/captura-movie.pcap ~/movie-cosecha.json` está listo y probado. Su primera ejecución real encontró 0 porque el filtro BPF que calculaba el offset TCP no es compatible con esta variante de PCAPdroid; se corrigió para filtrar simplemente `tcp dst port 80` (solicitudes salientes, nunca respuestas TS). Tras actualizar, volver a correr SOLO este análisis: no requiere ni modifica el PCAP ni pide otra captura. Deja `~/movie-pcap-orden.txt/.json` con manifests en orden y peticiones HTTP de portada/ficha que estén en claro, sin query/tokens/cookies.
...
- `ordenar-pcap-movie.js` — analiza el PCAP YA recibido mediante `tcpdump` y genera orden temporal de GET M3U8 + posibles requests HTTP de portada/ficha, con queries saneadas; no descarga video ni altera el PCAP. Sus reportes locales quedan ignorados por git.
