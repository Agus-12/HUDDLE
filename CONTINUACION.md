...
- **Orden de la captura sin recapturar:** `ordenar-pcap-movie.js ~/captura-movie.pcap ~/movie-cosecha.json` está listo y probado con un PCAP simulado. Usa `tcpdump` con filtro de paquetes HTTP GET/POST (no imprime los cientos de MB de TS), deja `~/movie-pcap-orden.txt/.json` con los manifests en orden y las peticiones HTTP de portada/ficha que estén en claro, sin query/tokens/cookies. Correrlo sobre el PCAP real y usar ese orden para contrastar lo que vio el amigo.
