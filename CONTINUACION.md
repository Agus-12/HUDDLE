...
- **Orden de la captura sin recapturar:** la versión binaria de `ordenar-pcap-movie.js` leyó el PCAP real correctamente: **413,435 paquetes, 39 manifests/39 rutas**, entre `00:13:39` y `00:28:54 UTC`, todos con host `movievn.j5t2n.com`; guarda el orden y 154 GET de TS. Ya no usar los dos filtros BPF previos. La siguiente versión también busca `GET /control?msg=verify`, descarta el device_id y conserva únicamente el app `vod_id`, que puede cruzarse con la caché de fichas y el manifest temporalmente cercano. No requiere ni modifica el PCAP ni pide otra captura.
...
3. La captura #1 YA está en Oracle (`~/captura-movie.pcap`, 636.6 MiB) y ya fue cosechada: 39 rutas → 6 M3U8 válidos (4:22:41.824) + 33 HTTP 403. `identificar-movie.js` ya dejó caché de 5,878 episodios y `ordenar-pcap-movie.js` ya sacó el orden de 39 manifests. No repetir captura ni borrar esos archivos. Tras la versión que extrae `vod_id` local, correr SOLO `node ordenar-pcap-movie.js ~/captura-movie.pcap ~/movie-cosecha.json`, leer la sección `VOD_ID DEL PROXY LOCAL VISIBLES` y cruzarla antes de guardar el mapa. La ruta `fb5b1ca540a1` no se integra aún: tag de audio tha/apertura tailandesa, pendiente ASR de un posible doblaje latino. No subir PCAP ni reportes crudos al repo público.
