...
### ⏳ EN CURSO — "LA COSECHA"
- **Bloqueo actual:** ya están las 39 carpetas `{id-12-hex}`; falta asociarlas con certeza a título/temporada/episodio. El id de carpeta es aleatorio (no derivable de ids públicos — ya se probó md5/sha1 de web-ep-id, web-vod-id, app-vod-id y combinaciones). La asociación puede venir de (a) `vod/info_new` cifrado TLS, (b) la cronología HTTP ya extraída, o (c) el `vod_id` que el proxy local pide en `/control?msg=verify` si PCAPdroid lo incluyó.
...
- **Resultado de validación de captura #1:** `cosechar-movie.js` revisó las 39 rutas: **6 M3U8 válidos / 33 HTTP 403**. Los 6 válidos suman **4:22:41.824** y duran: 43:27.100 (201 segmentos), 42:26.086 (196), 42:58.096 (199), 41:55.086 (194), 42:46.096 (197) y 49:09.360 (227). Los 403 son solo la respuesta actual del origen por IP desnuda: **no son prueba de que estén muertos** ni se borran; conservar la lista y no insistir sobre ellos ahora.
- **Identificar QUÉ episodio es cada carpeta:** `identificar-movie.js ~/movie-cosecha.json` consulta y cachea las fichas web, deduplica el catálogo histórico repetido, suma las 83 fichas actuales de la vitrina Telenovela pública y compara `vod_duration` con `EXTINF` (±12 s). La corrida real indexó **5,878 episodios** y encontró candidatos para 6/6, pero muchas novelas comparten exactamente esas duraciones (150–400 candidatos en varias rutas). La hipótesis por duración `Camino a Arcadia` T1E3 para `fb5b1ca540a1` queda **descartada**: su primer TS accesible trae tag de audio `tha` y una apertura tailandesa, no la novela española de ese candidato. No guardar ninguna asignación definitiva solo por duración.
- **Audio latino:** el primer TS de `fb5b1ca540a1` fue inspeccionado fuera del repo: H264/AAC, tag de audio `tha`, tarjetas iniciales tailandesas/subtítulos españoles. No incorporar esa ruta al catálogo latino salvo verificación ASR de un doblaje latino alterno; la metadata de idioma por sí sola no siempre basta.
- **Orden de la captura sin recapturar:** la versión binaria de `ordenar-pcap-movie.js` leyó el PCAP real correctamente: **413,435 paquetes, 39 manifests/39 rutas**, entre `00:13:39` y `00:28:54 UTC`, todos con host `movievn.j5t2n.com`; guarda el orden y 154 GET de TS. PCAPdroid no incluyó `/control?msg=verify`: 0 `vod_id` locales visibles. Ya no usar los dos filtros BPF previos. No requiere ni modifica el PCAP ni pide otra captura.
- **Recuperar manifests del propio PCAP:** `cosechar-pcap-movie.js ~/captura-movie.pcap ~/movie-cosecha.json` está listo para reconstruir las respuestas HTTP de los flujos TCP ya capturados y calcular `EXTINF` para las 39 rutas, incluso las 33 que hoy devuelven 403 al consultarlas de nuevo. Reporta por separado el HTTP/duración **capturados entonces** y la comprobación actual; no descarga CDN, no guarda el body M3U8 ni segmentos/tokens. Soporta el PCAP clásico de esta captura (`microsegundos`) y link types RAW/ethernet/VLAN/Linux cooked; probado con respuesta segmentada, conexión persistente y retransmisión. Pendiente correrlo en el PCAP real.

### 📋 PLAN DESPUÉS DE LA COSECHA
1. Construir en Huddle la sección "Movie": catálogo confirmado (la muestra local no es completa) + URLs cosechadas + reproductor que sirva los m3u8 re-escritos apuntando los segmentos a `http://147.124.216.142/vod/1/.../{NNNN}.ts`.
2. Estrategia de catálogo: empezar con los títulos favoritos del usuario (los que el amigo coseche) e ir creciendo con rondas de cosecha. Corregir por separado el recolector de catálogo: las 8,000 filas históricas no son 8,000 títulos. La automatización total de URLs requeriría revertir libpp_hls.so (ver dead ends — días en Ghidra, strings totalmente cifrados, "last resort").
...
3. La captura #1 YA está en Oracle (`~/captura-movie.pcap`, 636.6 MiB) y ya fue cosechada desde el CDN: 39 rutas → 6 M3U8 válidos (4:22:41.824) + 33 HTTP 403. `identificar-movie.js` ya dejó caché de 5,878 episodios y `ordenar-pcap-movie.js` ya sacó el orden de 39 manifests; no hubo `vod_id` de proxy local. No repetir captura ni borrar esos archivos. Siguiente paso: correr `node cosechar-pcap-movie.js ~/captura-movie.pcap ~/movie-cosecha.json` y leer `~/movie-cosecha-pcap.txt`: recupera duraciones de las respuestas HTTP existentes en la captura, no de la disponibilidad actual. Con las duraciones completas, buscar secuencias coherentes por fecha/orden antes de guardar el mapa. La ruta `fb5b1ca540a1` no se integra aún: tag de audio tha/apertura tailandesa, pendiente ASR de un posible doblaje latino. No subir PCAP ni reportes crudos al repo público.
