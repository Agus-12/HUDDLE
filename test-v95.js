/* E2E v95: PÓSTER DE LA SERIE EN CONTINUAR-VIENDO — la entrada de un
 * episodio guardaba el still de la escena; ahora lleva el póster de la
 * serie (como los animes) y el still se queda solo en el selector +
 * todo v94→v81. */
const fs = require('fs');
...
(async () => {
  ...
  ok(srv.includes("const UI_VERSION = 'v95'"), 'servidor en v95');
  ok(js.includes("const APP_VERSION = 'v95'"), 'cliente en v95');
  ok(idx.includes('id="verBadge">v95'), 'badge v95');
  ok(idx.includes('/app.js?v=95') && idx.includes('/style.css?v=95'), 'cache-busters v95');
  /* hls.js vendored */
  ...
  ok(listoP, 'Fundación 1x3 reproduce tras el selector');
  await pA.evaluate(() => document.querySelector('#soloBack').click());
  await sleep(1500);
  const cont = await pA.evaluate(async () => {
    await cargarContinuar();
    const card = [...document.querySelectorAll('#continueRow .cont-card')].find((x) => { const e = x.querySelector('.cont-ep'); return e && /1x3/.test(e.textContent); });
    if (!card) return null;
  ...
  ok(!!cont, 'la tarjeta de continuar del 1x3 existe');
  if (cont) {
