/* E2E v56: invitación con póster + volver a tu sala + tendencias semana */
...
(async () => {
  ...
  const weekVisible = await pA.$eval('#weekBox', (n) => !n.classList.contains('hidden'));
  if (weekVisible) {
    const cardsW = await pA.$$eval('#weekRow .sr-card', (n) => n.length);
    ok(cardsW >= 10, `fila Tendencias de la semana: ${cardsW} cards`);
  } else { ok(false, 'fila Tendencias de la semana visible'); }

  /* A toca la primera card de populares → sala con espejo */
