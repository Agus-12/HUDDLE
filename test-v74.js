/* E2E v74: botones de episodio anterior/siguiente para series (junto
 * a ampliar), "Buscar películas" en el último episodio, y pantalla de
 * espera en tarjeta (carátula + "Preparando Episodio N") — más todo
 * v73/v72/v71/v70 */
const fs = require('fs');
...
(async () => {
  ...
  /* v65 */
  ok(srv.includes("const UI_VERSION = 'v74'"), 'servidor en v74');
  ok(js.includes("const APP_VERSION = 'v74'"), 'cliente en v74');
...
ok(/\.play-btn\s*\{[^}]*justify-content:\s*center/s.test(css), 'v71: "Toca para empezar" centrado (estilo base, aplica también al celular)');
  ...
  ok(css.includes('.ep-nav') && css.includes('.peli-card'), 'v74: esquina de episodios + tarjeta de espera');
  ...
  if (listoAni) {
    ...
    /* siguiente → episodio 2 con pantalla de espera en tarjeta */
    ...
    ok(esp74.visible && /episodio 2/i.test(esp74.estado) && esp74.poster, `pantalla de espera en tarjeta: "${esp74.estado}" con carátula`);
