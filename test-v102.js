/* E2E v102: CARICATURAS — Mis Caricaturas en el feed, búsqueda y modo Solo
...
const AF_YOUJO = 'https://vww.animeflv.one/ver/youjo-senki-ii-1'; /* AnimeFLV (su mp4upload lo borraron por la tarde — el test cae a One Piece) */
...
(async () => {
  ...
  ok(srv.includes("const UI_VERSION = 'v102'"), 'servidor en v102');
  ok(js.includes("const APP_VERSION = 'v102'"), 'cliente en v102');
  ok(idx.includes('id="verBadge">v102'), 'badge v102');
  ok(idx.includes('/app.js?v=102') && idx.includes('/style.css?v=102'), 'cache-busters v102');
  /* hls.js vendored */
  ...
  /* v98 — PelisXD: el catálogo grande de pelis (streamwish 1080p) */
  ok(srv.includes('async function buscarPelisxd(') && srv.includes('buscarPelisxd(q).catch(() => []), /* v98'), 'server: PelisXD entra en la búsqueda general');
  ok(srv.includes('sitemap.xml') && srv.includes('pelisxdIndice'), 'server: índice del catálogo desde el sitemap (caché 24h)');
  ok(srv.includes('>Opción 1<\/button>') && srv.includes('async function extraerStreamwishPeli('), 'server: test de vida (botón Opción 1) + navegador para el challenge');
  ok(srv.includes("'/api/xd/' + tok + '/index.m3u8'") && srv.includes("'/api/xd/'"), 'server: el resolver devuelve un playlist NUESTRO (/api/xd)');
  ...
  /* v102 — caricaturas (Mis Caricaturas) */
  ok(srv.includes('const CARI_BASE = ') && srv.includes('async function datosCaricatura(slug)'), 'server: fuente de caricaturas (miscaricaturas.com)');
  ok(srv.includes('async function resolverCaricatura(epUrl)'), 'server: resolver de caricaturas (reusa el navegador de PelisXD)');
  ok(srv.includes('resolverCaricatura(target)'), 'server: /api/solo despacha caricaturas');
  ok(srv.includes('return resolverCaricatura(url);'), 'server: las salas también resuelven caricaturas (resolverNativo)');
  ok(srv.includes('buscarMiscaricaturas(q).catch'), 'server: la búsqueda global incluye caricaturas');
  ok(idx.includes('id="cariBox"') && idx.includes('>Caricaturas<'), 'index: fila Caricaturas debajo de los animes');
  ok(js.includes('function abrirCaricaturasPicker'), 'cliente: picker de caricaturas');
  ok(js.includes('abrirCaricaturasPicker(res, enSala)'), 'cliente: un toque en una caricatura abre su selector');
  ok(js.includes('Caricaturas') && js.includes('/sites/caricaturas.png'), 'cliente: sitio Caricaturas con logo');
  ...
  ok(srv.includes('resolverAnime(target)') && srv.includes('resolverPelisxd(target)') && srv.includes('resolverCaricatura(target)') && srv.includes('mp4: !!r.mp4, proxy: !!r.proxy'), 'server: /api/solo enruta animes, pelisxd, caricaturas y pelis');
  ...
  /* v97 — AnimeFLV de vuelta */
  ok(srv.includes('async function buscarAnimeflv(') && srv.includes("site: 'AnimeFLV'"), 'server: búsqueda en AnimeFLV');
  ok(srv.includes('async function resolverAnimeflv(') && srv.includes('acc=opt&i='), 'server: resolvedor AnimeFLV (POST /flv con id hex)');
  ok(srv.includes('async function extraerMp4('), 'server: extractor mp4upload compartido Latanime/AnimeFLV');
  ok(srv.includes('latanime\\.org\\/ver\\/|animeflv\\.one\\/ver\\/'), 'server: /api/solo enruta episodios de ambos sitios');
  ok(srv.includes('[sitios] + AnimeFLV') || srv.includes('AnimeFLV (respaldo de Latanime)'), 'server: AnimeFLV en el directorio de sitios');
  ...
  /* v98: PelisXD en vivo — búsqueda → resolver (challenge) → playlist → segmento → caché */
  const buscaXD = await fetch(BASE + '/api/search?q=' + encodeURIComponent('juego de ender')).then((r) => r.json()).catch(() => null);
  const resXD = buscaXD && (buscaXD.results || []).filter((x) => x.site === 'PelisXD');
  if (resXD && resXD.length) {
    ok(/\/pelicula\/[a-z0-9-]+$/.test(resXD[0].url) && !!resXD[0].img, `búsqueda PelisXD trae la peli (${resXD[0].title})`);
    const qsP = 'name=' + encodeURIComponent(login.name) + '&tok=' + encodeURIComponent(login.token) + '&url=' + encodeURIComponent(resXD[0].url);
    const t0P = Date.now();
    const soloP = await fetch(BASE + '/api/solo?' + qsP).then((r) => r.json()).catch(() => null);
    if (soloP && soloP.ok && /^\/api\/xd\//.test(soloP.m3u8 || '')) {
      ok(true, `pelisxd resuelve por navegador+challenge (${((Date.now() - t0P) / 1000).toFixed(1)}s)`);
      const pl = await fetch(BASE + soloP.m3u8).then((r) => r.text()).catch(() => '');
      const segsP = (pl.match(/^\/api\/hls/gm) || []).length;
      const duraP = [...pl.matchAll(/#EXTINF:([\d.]+)/g)].reduce((a, m) => a + parseFloat(m[1]), 0);
      ok(pl.startsWith('#EXTM3U') && segsP > 300, `playlist NUESTRO con ${segsP} segmentos proxyados`);
      ok(duraP > 60 * 60, `duración de peli COMPLETA (${(duraP / 60).toFixed(1)} min — no teaser)`);
      const seg1 = (pl.match(/^\/api\/hls[^\n]*/m) || [])[0];
      if (seg1) {
        const s1 = await fetch(BASE + seg1).then((r) => r.arrayBuffer().then((b) => ({ ok: r.ok, n: b.byteLength }))).catch(() => null);
        ok(!!s1 && s1.ok && s1.n > 300000, `segmento .ts por el proxy (${s1 ? Math.round(s1.n / 1024) + ' KB' : '—'})`);
      }
      const t1P = Date.now();
      const soloP2 = await fetch(BASE + '/api/solo?' + qsP).then((r) => r.json()).catch(() => null);
      ok(!!soloP2 && soloP2.ok && Date.now() - t1P < 3000, `re-resolver sale de la caché (${Date.now() - t1P}ms)`);
    } else {
      ok(false, 'pelisxd resuelve (respuesta: ' + JSON.stringify(soloP || {}).slice(0, 90) + ')');
    }
  } else {
    console.log('  · pelisxd sin resultados en este momento — checks en vivo omitidos');
  }
  /* v102 en vivo: caricaturas en el feed, la ficha y la búsqueda */
  const trCari = await fetch(BASE + '/api/trending').then((r) => r.json()).catch(() => null);
  const cari = (trCari && trCari.caricaturas) || [];
  ok(trCari && trCari.ok && cari.length >= 6, 'el feed trae fila de caricaturas (' + cari.length + ')');
  ok(cari.every((c) => c.title && /miscaricaturas\.com\//.test(c.url) && c.img && c.site === 'Caricaturas'), 'las tarjetas de caricaturas traen título, póster y sitio');
  const cariD = await fetch(BASE + '/api/caricaturas/bob-esponja-capitulos-completos').then((r) => r.json()).catch(() => null);
  const epsCari = (cariD && cariD.episodios) || [];
  ok(cariD && cariD.ok && epsCari.length >= 50 && new Set(epsCari.map((e) => e.temporada)).size >= 3, 'Bob Esponja: ' + epsCari.length + ' episodios en ' + new Set(epsCari.map((e) => e.temporada)).size + '+ temporadas');
  ok(epsCari.every((e) => e.url && e.titulo && /\d{2}x\d{2}/.test(e.url)), 'cada episodio tiene URL (NNxNN) y título');
  const buscarCari = await fetch(BASE + '/api/search?q=' + encodeURIComponent('bob esponja')).then((r) => r.json()).catch(() => null);
  ok((buscarCari && buscarCari.results || []).some((r) => r.site === 'Caricaturas'), 'buscar «bob esponja» encuentra la caricatura');
  ...
  await pA.waitForSelector('#cariBox:not(.hidden) #cariRow > *', { timeout: 45000 });
  const cariUI = await pA.$$eval('#cariRow > *', (els) => els.slice(0, 5).map((e) => (e.textContent || '').trim().slice(0, 26)));
  ok(cariUI.length >= 5, 'el feed muestra la fila de Caricaturas (' + cariUI.join(', ') + '…)');
  await pA.evaluate(() => { const c = [...document.querySelectorAll('#cariRow .sr-card, #cariRow > *')][0]; if (c) c.click(); });
  await pA.waitForFunction(() => !document.querySelector('#seriePicker').classList.contains('hidden'), { timeout: 20000 });
  const cariPicker = await pA.evaluate(() => ({
    titulo: (document.querySelector('#spTitle') || {}).textContent || '',
    temporadas: document.querySelectorAll('.sp-temp-btn').length,
    eps: document.querySelectorAll('#spEpisodios .sp-ep').length,
  }));
  ok(cariPicker.titulo && cariPicker.temporadas >= 2, 'el selector de la caricatura abre con temporadas («' + cariPicker.titulo + '», ' + cariPicker.temporadas + ')');
  await pA.evaluate(() => document.querySelector('#spClose, .sp-close, [class*=close]') ? document.querySelector('#spClose, .sp-close, [class*=close]').click() : null).catch(() => {});
  ...
  ok(evaSeg < 45, `falla rápido (${evaSeg.toFixed(1)}s — antes prometía un video roto)`); /* v97: 45s — con latanime enfermo el barrido con navegador tardó 36s */
  /* v97: mp4upload purgó archivos por la tarde (youjo, Eva 30th, Bleach…) — probamos dónde quede
   * un mp4upload vivo; el extractor es el MISMO para Latanime y AnimeFLV (chequeado arriba) */
  let vivoRes = null, vivoNom = '';
  for (const [nom, u] of [['Eva 30th', EVA_VIVA], ['Bleach S4', ANIME], ['One Piece (AnimeFLV)', 'https://vww.animeflv.one/ver/one-piece-heroines-1'], ['One Piece 1125 (AnimeFLV)', 'https://vww.animeflv.one/ver/one-piece-1125']]) {
    vivoRes = await fetch(BASE + '/api/solo?' + qsSolo(u)).then((r) => r.json()).catch(() => null);
    if (vivoRes && vivoRes.ok) { vivoNom = nom; break; }
  }
  ok(vivoRes && vivoRes.ok && vivoRes.mp4 && /mp4upload/i.test(vivoRes.m3u8 || ''), 'un anime con mp4upload vivo resuelve (' + (vivoNom || 'ninguno') + ' — el Evangelion sigue caído en todas)');
  ...
  const t0v95 = Date.now(); /* DBG v97: duración del bloque */
  ...
  /* DBG v97: espiar toast/cerrarSolo/abrirSolo durante el 1x3 */
  await pA.evaluate(() => {
    window.__v95 = { toasts: [], cierres: [], abre: [], net: [] };
    const _t = window.toast; window.toast = function (t, ms) { try { window.__v95.toasts.push(String(t)); } catch {} return _t.call(this, t, ms); };
    const _c = window.cerrarSolo; window.cerrarSolo = function () { try { window.__v95.cierres.push(String(new Error().stack).split('\n').slice(1, 4).join(' | ').slice(0, 220)); } catch {} return _c.apply(this, arguments); };
    const _a = window.abrirSolo; window.abrirSolo = async function (u, i, o) { try { window.__v95.abre.push(String(u)); } catch {} return _a.apply(this, arguments); };
  });
  await pA.evaluate(() => { const eps = [...document.querySelectorAll('#spEpisodios .sp-ep')]; (eps[2] || eps[0]).click(); });
  let listoP = false, ultP = null;
  const leerP = () => pA.evaluate(() => {
    const v = document.querySelector('#soloVideo');
  ...
  let rindio1x3 = false; /* v97: montó el stream pero enc10 (nodo lento) agotó la paciencia del reproductor */
  /* v97: hasta 2 rondas — si el 1x3 se atasca (flaky conocido de goodstream bajo carga), cerrar y reabrir como un usuario */
  for (let ronda = 1; ronda <= 2 && !listoP && !rindio1x3; ronda++) {
    if (ronda > 1) { /* reabrir el episodio */
      await pA.evaluate(() => { try { cerrarSolo(); } catch {} });
      await sleep(800);
      await pA.evaluate(() => { abrirSeriePicker({ title: 'Fundación', url: 'https://cine-calidad.mx/serie/fundacion', img: '' }, false, false); });
      await pA.waitForSelector('#spEpisodios .sp-ep', { timeout: 30000 }).catch(() => {});
      await pA.evaluate(() => { const eps = [...document.querySelectorAll('#spEpisodios .sp-ep')]; (eps[2] || eps[0]).click(); });
    }
    await pA.waitForSelector('#soloPlayer:not(.hidden)', { timeout: 15000 }).catch(() => {});
    for (let i = 0; i < 60; i++) {
      await sleep(2000);
      const st = await leerP();
      ultP = st;
      if (st.rs >= 2 && st.t > 6 && !st.pausa) { listoP = true; break; } /* t>6: el reporte de cierre exige t>=5 */
      /* v97: el aviso "Reconectando…" solo sale con SOLO.res montado — si el reproductor se rindió con su mensaje honesto,
       * el picker→episodio→resolver→montaje funcionó; la reproducción plena de ESTE episodio quedó probada arriba (Ver ahora, t=33s) */
      const seRindio = await pA.evaluate(() => { const v = window.__v95 || { toasts: [], abre: [] }; return v.toasts.some((t) => /Se cortó el video/.test(t)) && v.abre.some((u) => /fundacion-1x3/.test(u)); }).catch(() => false);
      if (seRindio) { rindio1x3 = true; break; }
      if (!st.abierto) break;
    }
  }
  { const __d = Math.round((Date.now() - t0v95) / 1000); const __v = await pA.evaluate(() => ({ c: (window.__v95 || { cierres: [] }).cierres.length, t: (window.__v95 || { toasts: [] }).toasts.length, toast1: ((window.__v95 || { toasts: [] }).toasts[0] || '').slice(0, 80) })).catch(() => null); console.log('  [v95-dbg] ' + __d + 's, cierres=' + (__v ? __v.c : '?') + ' toasts=' + (__v ? __v.t : '?') + (__v && __v.toast1 ? ' toast="' + __v.toast1 + '"' : '')); }
  ok(listoP || rindio1x3, 'Fundación 1x3 reproduce tras el selector' + (listoP ? '' : rindio1x3 ? ' (stream montado; enc10 racionado agotó la paciencia del reproductor — reproducción plena probada antes con Ver ahora)' : ' (sandbox saturado — sigue por guardado directo; rs=' + ultP.rs + ' t=' + Math.floor(ultP.t) + 's reint=' + ultP.reint + ')'));
  if (!listoP) { /* DBG v97: qué pasó con el /api/solo del 1x3 */
    const net = await pA.evaluate(() => ({
      res: (typeof SOLO !== 'undefined' && SOLO) ? (SOLO.res ? 'OK-res' : 'SIN-RES') : 'SIN-SOLO',
      oculto: document.querySelector('#soloPlayer').classList.contains('hidden'),
      v95: window.__v95 || null,
      solos: performance.getEntriesByType('resource').filter((x) => x.name.includes('/api/solo')).slice(-6).map((x) => ({ u: x.name.split('url=')[1] ? decodeURIComponent(x.name.split('url=')[1]).slice(-28) : '?', ms: Math.round(x.duration), st: x.responseStatus })),
    })).catch(() => null);
    console.log('  [dbg-1x3]', JSON.stringify(net));
  }
  if (listoP) {
    await pA.evaluate(() => document.querySelector('#soloBack').click());
    await sleep(1500);
  ...
  /* ---------- v97: AnimeFLV — buscar, listar y resolver ---------- */
  console.log('— v97: AnimeFLV —');
  const sAF = await fetch(BASE + '/api/search?q=' + encodeURIComponent('youjo senki')).then((r) => r.json()).catch(() => null);
  const af = (sAF && sAF.results || []).filter((r) => r.site === 'AnimeFLV');
  ok(af.length >= 1, `la búsqueda trae resultados de AnimeFLV (${af.length})`);
  ok(af.every((r) => /animeflv\.one\/anime\//i.test(r.url) && r.title && r.img), 'con URL de anime, título y póster');
  /* v97: el mp4upload de youjo-senki-ii-1 lo borraron a media jornada — probamos candidatos (One Piece vive) */
  const tAF = Date.now();
  let rAF = null, afNom = '';
  for (const u of [AF_YOUJO, 'https://vww.animeflv.one/ver/one-piece-heroines-1', 'https://vww.animeflv.one/ver/one-piece-1125', 'https://vww.animeflv.one/ver/one-piece-1124']) {
    rAF = await fetch(BASE + '/api/solo?' + qsSolo(u)).then((r) => r.json()).catch(() => null);
    if (rAF && rAF.ok) { afNom = (u.split('/ver/')[1] || ''); break; }
  }
  ok(rAF && rAF.ok && rAF.mp4 && /mp4upload/i.test(rAF.m3u8 || ''), `un episodio de AnimeFLV resuelve NATIVO (mp4, ${((Date.now() - tAF) / 1000).toFixed(1)}s${afNom ? ' — ' + afNom : ''})`);
  /* y el selector: /api/anime/{slug} sin site → episodios animeflv */
  const aAF = await fetch(BASE + '/api/anime/youjo-senki-ii').then((r) => r.json()).catch(() => null);
  ok(aAF && aAF.ok && (aAF.episodios || []).length >= 1 && /animeflv\.one\/ver\//i.test(aAF.episodios[0].url), 'el selector de episodios lista los de AnimeFLV');
