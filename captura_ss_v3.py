# captura_ss.py v3 — captura TODO + CUERPOS de los POST (ahí viaja el sign de info_new)
#   + respuestas completas (las URLs de los capítulos vienen en la respuesta de info_new)
# Se usa con: mitmdump -s captura_ss_v3.py --listen-host 0.0.0.0 --listen-port 8080 --set block_global=false
import re, base64, subprocess
from mitmproxy import http

PATRON = re.compile(r'z3azky|vd7au6|j5t2n|k5ca|freemovies|d9s3x4|movievn|u9m2|r2c7a0|4j4damaqa|k0j5n7|freecine|h4c5|maqbc', re.I)
...
def response(flow: http.HTTPFlow):
    ...
    try:
        ...
        pass
    # v3: CUERPO del request (aquí viaja el sign de info_new)
    try:
        cuerpo_req = flow.request.get_text(strict=False) or ''
        if cuerpo_req:
            print('>>>   BODY %s' % cuerpo_req[:400])
    except Exception:
        pass
    cuerpo = ''
    try:
