/* E2E v67: sección "Animes del momento" en el inicio (estrenos de
 * Latanime con carátulas) + todo lo de v66 (play al centro, botones
 * sobre la barrita) + v65 (Latanime predeterminado, logos propios) +
 * tira de mensajes sobre los controles + v64 (bypass goodstream) */
const fs = require('fs');
...
(async () => {
  ...
  /* v65 */
  ok(srv.includes("const UI_VERSION = 'v67'"), 'servidor en v67');
  ok(js.includes("const APP_VERSION = 'v67'"), 'cliente en v67');
  ...
  ok(!css.includes('.pseudo-fs) .play-btn'), 'CSS v66: el botón de play se queda al CENTRO también en vista normal');
  ...
  ok(/\/api\/img\?u=/.test(aniPk.poster), `póster vía proxy propio (${aniPk.poster.slice(0, 46)}…)`);
  await pA.evaluate(() => cerrarSeriePicker());
  await sleep(400);
