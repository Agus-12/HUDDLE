/* E2E v71: pantalla del celular como siempre (compacta) con el
 * "Toca para empezar" CENTRADO en medio del reproductor — más todo
 * v70 (búsqueda Cuevana+Latanime, sin AnimeD23) y v69/v68 */
const fs = require('fs');
...
(async () => {
  ...
  /* v65 */
  ok(srv.includes("const UI_VERSION = 'v71'"), 'servidor en v71');
  ok(js.includes("const APP_VERSION = 'v71'"), 'cliente en v71');
...
ok(/\.play-btn\s*\{[^}]*justify-content:\s*center/s.test(css), 'v71: "Toca para empezar" centrado (estilo base, aplica también al celular)');
  ...
  const posM = await pM.evaluate(() => {
    ...
    return {
      ...
      shellT: S.top, shellB: S.bottom, shellH: S.height, barB: W.bottom,
  ...
  ok(posM.shellB - posM.barB < 40, `barrita al fondo del reproductor (${Math.round(posM.shellB - posM.barB)}px)`);
