...
function abrirSeriePicker(res, enSala, esAnime, esGp) {
  ...
  fetch((esMovie ? '/api/movie/ficha/' + slug : esGp ? '/api/gopelis/' + slug : esEnp ? '/api/enp/' + slug : esNv ? '/api/novelas/' + slug : esAnime ? ('/api/anime/' + slug + (esLat ? '?site=latanime' : '')) : '/api/serie/' + slug)).then((r) => r.json()).then((d) => { /* v206 + v206.2 + v207 */
