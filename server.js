...
try {
  ...
  if (_rl) {
    ...
    else { const _mR = String(_rl).match(/https?:\/\/[^\s)\]]+/); CDN_RELAY = _mR ? _mR[0] : _rl; console.log('[relay] Cargado de disco:', CDN_RELAY, (_mR && _mR[0] !== _rl) ? '(v350: saneado)' : ''); }
  }
} catch {}
const RELAY_SEMA = { n: 0, cola: [] }; /* v350: máx 3 en vuelo — el uplink de casa es finito (sonda + reproducción NO lo ahogan; el resto hace fila de ms) */
async function fetchRelay(url, ms) {
  if (!CDN_RELAY) return fetchSeguro(url, ms);
  if (RELAY_SEMA.n >= 3) await new Promise((libre) => RELAY_SEMA.cola.push(libre));
  RELAY_SEMA.n++;
  try { return await fetchRelayInterno(url, ms); } finally { RELAY_SEMA.n--; const _s = RELAY_SEMA.cola.shift(); if (_s) _s(); }
}
async function fetchRelayInterno(url, ms) { /* v236: fetch a través del relay — NUNCA cae al directo (el token se liga a la IP) */
  if (!CDN_RELAY) return fetchSeguro(url, ms);
...
async function bovedaAutoSeries() {
  try {
    ...
    if (memMB > 320) { if (Date.now() - (BOVEDA_AUDIT.memAvisoAt || 0) > 3600000) { BOVEDA_AUDIT.memAvisoAt = Date.now(); console.log('[boveda-sonda] pausada — memoria alta (' + Math.round(memMB) + ' MB > 320)'); } return; } /* v349: el silencio es un síntoma — ahora se sabe POR QUÉ */
...
async function sondaBoveda() {
  try {
    ...
    for (let n = 0; n < CANARIOS_VIMEOS.length && !canario; n++) {
      ...
      canario = await canarioPruebaRapida(CANARIOS_VIMEOS[i]).catch(() => false); /* v350 */
    ...
    /* v348: AUDITORÍA DE SERIES cq — punto ciego desde v321 (solo se auditaban
     * películas). 2 entradas por ciclo, 1 episodio al azar de cada una;
     * 2 strikes = ESE CÓDIGO fuera (la serie y sus demás episodios quedan). */
    for (let muestraS = 0; muestraS < 2; muestraS++) {
      let parS = null;
      for (const [k, v] of BOVEDA) { if (!/^(cq:tvshow|cq:anime):/.test(k) || !v.eps || BOVEDA_AUDIT.revisados.has(k)) continue; parS = [k, v]; break; }
      if (!parS) break;
      const [k, v] = parS;
      BOVEDA_AUDIT.revisados.add(k);
      const Ts = Object.keys(v.eps);
      const T = Ts[Math.floor(Math.random() * Ts.length)];
      const Es = T && Object.keys(v.eps[T] || {});
      const E = Es && Es.length ? Es[Math.floor(Math.random() * Es.length)] : null;
      const codeS = E ? v.eps[T][E] : null;
      if (!codeS) continue;
      let sirvioS = false;
      ...
      if (sS >= 2) {
        delete v.eps[T][E];
        if (!Object.keys(v.eps[T]).length) delete v.eps[T];
        if (!Object.keys(v.eps).length) BOVEDA.delete(k); else bovedaGuardar();
        BOVEDA_AUDIT.sospechosos.delete(k);
        sondaNotify('Bóveda', 'muerto', v.t || k, (v.t || k) + ' T' + T + 'E' + E + ' — código podrido confirmado 2× — fuera de la bóveda (la serie queda)');
        console.log('[boveda-sonda] serie ' + (v.t || k) + ' T' + T + 'E' + E + ' podrido confirmado: código fuera');
      } else console.log('[boveda-sonda] serie ' + (v.t || k) + ' T' + T + 'E' + E + ' no respondió (1ª vez)');
    }
    ...
    for (let muestraL = 0; muestraL < 3; muestraL++) {
      ...
      if (!par) break;
      const [k, v] = par;
      let vivo = false; /* v351: MULTI-HOSTER — se prueban hasta 2 embeds DISTINTOS (goodstream/no-vimeos primero): una entrada con varios hosters solo muere si TODOS están muertos */
      const ordenL = [...v.embeds].sort((a, b) => (/goodstream\.one/i.test(b || '') ? 2 : /vimeos/i.test(b || '') ? 0 : 1) - (/goodstream\.one/i.test(a || '') ? 2 : /vimeos/i.test(a || '') ? 0 : 1));
      for (const euL of ordenL.slice(0, 2)) {
        for (let i = 0; i < 2 && !vivo; i++) {
          try { const rL = await fetchSeguro(euL, 10000); vivo = !!(rL && (rL.ok || rL.status === 206)); if (!vivo && rL && rL.body) { try { await rL.body.cancel(); } catch {} } } catch {}
...
async function bovedaAutoCaps() {
  try {
    ...
    const FUENTES = ['lat', 'enn', 'd23', 'misc', 'lct', 'flv', 'dan', 'pxd']; /* v354: + pelisxd (auto-cosecha); flv y dan llevan flujo propio */
    const f = FUENTES[bovedaCaps.turno++ % FUENTES.length];
    const e = bovedaCapsEstado(f);
    if (f === 'flv') { /* v354: ANIMEFLV — sondeo secuencial /ver/<slug>-N; la página trae data-dwn: la LISTA COMPLETA de servidores (byse/dood/mp4upload… hosters independientes de vimeos) sin AJAX. La ficha no trae /ver/ (los monta JS) — por eso las fases comunes nunca cosecharon */
      if (!e.series.length) { const rs = await fetchSeguro('https://vww.animeflv.one/sitemap.xml', 15000).catch(() => null); const xs = rs && rs.ok ? await rs.text().catch(() => '') : ''; e.series = [...new Set([...xs.matchAll(/<loc>https:\/\/vww\.animeflv\.one\/anime\/([a-z0-9-]+)<\/loc>/g)].map((m) => m[1]))].sort(() => Math.random() - 0.5).slice(0, 600); }
      if (!e.series.length) return;
      if (!e.flvN) { e.flvN = 1; e.flvMiss = 0; }
      const serieF = e.series[e.si % e.series.length];
      const urlEp = 'https://vww.animeflv.one/ver/' + serieF + '-' + e.flvN;
      const r1F = await fetchSeguro(urlEp, 12000).catch(() => null);
      if (!r1F || !r1F.ok) { e.flvMiss = (e.flvMiss || 0) + 1; if (e.flvMiss >= 2) { e.si++; e.flvN = 1; e.flvMiss = 0; } return; } /* fin de la serie → siguiente */
      e.flvMiss = 0;
      const hhF = await r1F.text().catch(() => '');
      const tituloF = (((/<title>([^<]+)</i.exec(hhF) || [])[1] || '').replace(/^Ver\s+/i, '').replace(/\s*Sub\s+Espa[ñn]ol.*$/i, '').replace(/\s*Episodio\s+\d+.*$/i, '').replace(/\s*[\|–-]\s*AnimeFLV.*/i, '').trim()) || serieF; /* v354: título de serie limpio */
      const dwnF = /data-dwn="([^"]+)"/.exec(hhF);
      let embedsF = [];
      if (dwnF) {
        try {
          const arrF = JSON.parse(dwnF[1].replace(/&quot;/g, '"').replace(/\\\//g, '/'));
          embedsF = arrF.filter((u) => /^https?:\/\//i.test(u)).sort((a, b) => (/byse|dood|mp4upload/i.test(b) ? 1 : 0) - (/byse|dood|mp4upload/i.test(a) ? 1 : 0)).slice(0, 3); /* hosters independientes primero */
        } catch {}
      }
      e.flvN = e.flvN + 1;
      if (embedsF.length) { const claveF = 'flv:' + urlEp; if (!BOVEDA.has(claveF)) { bovedaPon(claveF, { serie: tituloF, poster: '', serieSlug: serieF, embeds: embedsF }); console.log('[boveda-caps] flv: +' + tituloF + ' E' + (e.flvN - 1) + ' (' + embedsF.length + ' servidores)'); } }
      return;
    }
    if (f === 'pxd') { /* v354: PELISXD en la rotación — auto-cosecha sin depender de que alguien reproduzca (1 GET por peli da título + embeds) */
      if (!e.series.length) { try { e.series = (await pelisxdIndice()).slice(); } catch {} if (!e.series.length) return; }
      const slugP = e.series[e.si % e.series.length];
      const urlP = 'https://www.pelisxd.com/pelicula/' + slugP;
      try {
        const r1P = await fetchSeguro(urlP, 12000);
        if (r1P.ok) {
          const hhP = await r1P.text();
          const tP = (((/<title>([^<]+)</i.exec(hhP) || [])[1] || '').replace(/^Ver\s+/i, '').replace(/\s*\(?\d{4}\)?\s*Online.*/i, '').replace(/\s*[\|–-]\s*PelisXD.*/i, '').trim()).slice(0, 90);
          const vSources = [];
          const reP = /v_source[^A-Za-z0-9]{0,12}([A-Za-z0-9+/=]{24,})/g;
          let m2P;
          while ((m2P = reP.exec(hhP))) { try { const u2 = Buffer.from(m2P[1], 'base64').toString('utf8'); if (/^https?:\/\//i.test(u2)) vSources.push(u2); } catch {} }
          if (vSources.length) { if (!BOVEDA.has('pxd:' + slugP)) { bovedaPon('pxd:' + slugP, { t: tP, embeds: vSources.slice(0, 3) }); console.log('[boveda-caps] pxd: +' + tP + ' (' + Math.min(vSources.length, 3) + ' embeds)'); } e.si++; return; }
        }
      } catch {}
      e.si++; return; /* página sin reproductores o falló — siguiente */
    }
    if (f === 'dan') { /* v335: DANIMADOS — flujo propio: catálogo en memoria (DANI_CAT) + sondeo secuencial /episodios/<slug>-TxN/ */
      ...
      if (!e.series.length) return;
      if (!e.daniT) { e.daniT = 1; e.daniN = 1; e.daniMiss = 0; } /* v354: BUG v335 — sin esto la URL era undefinedxundefined y dan JAMÁS cosechó (chip ausente en el panel) */
...
async function cqEmbedSirve(code) {
  ...
  for (let i = 0; i < 3; i++) { /* v314.1: 3 intentos como el player — los nodos muertos no condenan */
    ...
    try {
      ...
      huboM3u8 = true;
      if (await sirveElVideo(m3, 'https://vimeos.net/')) { regSaludV(m3, true); return true; }
      regSaludV(m3, false);
    } catch {}
  }
  if (CDN_RELAY) { /* v349: embed por el relay de casa — sin esto la sonda no puede juzgar NADA cuando vimeos bloquea el datacenter */
    try {
      const em = await (await fetchRelay('https://vimeos.net/embed-' + code + '.html', 15000)).text();
      const out = desempacar(em);
      const m3 = out && (out.match(/https?:\/\/[^"'\s\\]+\.m3u8[^"'\s\\]*/i) || [])[0];
      if (m3) { if (await sirveElVideo(m3, 'https://vimeos.net/')) { regSaludV(m3, true); return true; } regSaludV(m3, false); huboM3u8 = true; }
...
/* v350: prueba de canario RÁPIDA — directo y relay EN PARALELO (antes: 3
 * directos pesados + relay EN SECUENCIA por código ⇒ la verificación tardaba
 * un minuto y ahogaba el uplink de casa; hoy: veredicto en ~3-6 s) */
async function canarioPruebaRapida(code) {
  const reM = /https?:\/\/[^"'\s\\]+\.m3u8[^"'\s\\]*/i;
  const intentoDirecto = (async () => {
    try {
      const em = await fetchTexto('https://vimeos.net/embed-' + code + '.html', 'https://www.cinecalidad.am/');
      const m3 = (String(desempacar(em) || '').match(reM) || [])[0];
      if (m3 && await sirveElVideo(m3, 'https://vimeos.net/')) return true;
    } catch {}
    return false;
  })();
  const intentoRelay = (async () => {
    if (!CDN_RELAY) return false;
    try {
      const em = await (await fetchRelay('https://vimeos.net/embed-' + code + '.html', 15000)).text();
      const m3 = (String(desempacar(em) || '').match(reM) || [])[0];
      if (!m3) return false;
      try { const rv = await fetchRelay(m3, 10000); if (rv.ok || rv.status === 206) return true; } catch {}
    } catch {}
    return false;
  })();
  return (await Promise.all([intentoDirecto, intentoRelay])).some(Boolean);
}
/* v354: SALUD DE VIMEOS — censo de bodegas (s8, s10…) visto desde las
 * resoluciones reales + registro de fallos recientes para el panel. Así el
 * usuario ve EN VIVO qué bodegas están caídas y qué títulos fallan, sin
 * preguntar. */
const VIMEOS_SALUD = new Map(); /* bodega → {ok, ko, lastOk, lastKo} */
let VIMEOS_EST = { at: 0, vivo: false };
const bodegaDe = (m3) => { try { const u = new URL(m3); const mS = /(?:^|\.)s(\d+)\./i.exec(u.hostname); if (mS) return 's' + mS[1]; const mP = /[?&]srv=s(\d+)/.exec(u.search); if (mP) return 's' + mP[1]; return u.hostname; } catch { return '?'; } };
const FALLOS_RECIENTES = [];
/* v355: CENSO — qué bodega vive cada título (persistente; se llena solo al
 * reproducir/auditar). Así el panel dice CUÁNTOS títulos afecta una bodega. */
const CENSO_PATH = path.join(DATA_DIR, 'bodegas-censo.json');
const CENSO = { titulos: {} };
...
async function vimeosCanario() {
  ...
  for (let n = 0; n < CANARIOS_VIMEOS.length && !vimeosCanarioOk; n++) {
    ...
    try { vimeosCanarioOk = await canarioPruebaRapida(CANARIOS_VIMEOS[i]).catch(() => false); } catch { vimeosCanarioOk = false; } /* v350: en segundos, sin ahogar el relay */
  ...
  VIMEOS_EST = { at: vimeosCanarioAt, vivo: vimeosCanarioOk }; /* v354 */
...
async function resolverVimeos(embed, pageUrl) {
  ...
  const pedirEmbed = async (desfase, permitirRelay) => {
    ...
    try {
      ...
      if (!sirve) { regSaludV(m3u8, false); console.log('[vimeos] nodo ' + (new URL(m3u8).hostname) + ' no contestó (master+variante) — probando otro'); return { sin: 'red' }; }
      regSaludV(m3u8, true); /* v354: bodega viva vista en producción */
      return m3u8;
  ...
  if (!elegido) {
    ...
    if (!canarioVivo && CDN_RELAY) { /* v343: ÚLTIMA LÍNEA — el relay de casa ANTES de declarar saturado (si vimeos bloquea datacenters, por casa SÍ llega) */
      await espera(600); rs = await oleada(4, true); elegido = rs.find((r) => typeof r === 'string' && r) || null;
      if (elegido) console.log('[vimeos] SALVADO por el relay de casa');
    }
    if (!elegido && !canarioVivo) throw new Error('vimeos está saturado en este momento — reintenta en un minuto'); /* ventana mala completa */
    if (!elegido && !huboRed) throw new Error('este código ya no existe en vimeos'); /* todos dieron cuerpo sin video y el canario vivo = archivo vencido */
  }
  ...
  if (!elegido && CDN_RELAY) { await espera(800); rs = await oleada(4, true); elegido = rs.find((r) => typeof r === 'string' && r) || null; if (elegido) console.log('[vimeos] SALVADO por el relay de casa'); } /* ¿vimeos le bloquea la IP al servidor? el relay de casa */
  if (!elegido) throw new Error('vimeos está saturado en este momento — reintenta en un minuto'); /* v345: el relay es el ÚLTIMO intento — fuera la 3ª oleada directa (60+ s de nada cuando vimeos bloquea datacenters) */
  if (elegido) { /* v346: DOBLE CHEQUEO del ganador — hay nodos inestables (vimeos.zip hoy) que contestan el índice, ganan la carrera y dan 502 al reproducir. Re-verificamos y, si se murió, pasamos al SIGUIENTE ganador de la misma oleada (cero viajes nuevos) */
    const listaG = [...new Set(rs.filter((r) => typeof r === 'string' && r))];
    let final = null;
    for (const cand of listaG) { if (await m3u8Sirve(cand, true)) { final = cand; break; } console.log('[vimeos] ganador inestable detectado (2º chequeo falló) — probando el siguiente nodo'); }
    if (!final) throw new Error('vimeos está saturado en este momento — reintenta en un minuto'); /* todos los ganadores se murieron */
    elegido = final;
  }
...
async function bovedaRespaldo(titulo) {
  ...
  for (const [clave, v] of BOVEDA) {
    ...
    const ordenE = [...v.embeds].sort((a, b) => (b && /goodstream\.one/i.test(b) ? 2 : /vimeos/i.test(b || '') ? 0 : 1) - (a && /goodstream\.one/i.test(a) ? 2 : /vimeos/i.test(a || '') ? 0 : 1)); /* v351: DIVERSIDAD — goodstream primero, luego otros hosters, vimeos al final (familia vimeos = el punto débil de hoy) */
    for (const eu of ordenE) {
      try {
        ...
        if (out && out.m3u8) { console.log('[boveda] RESPALDO: "' + titulo + '" vía ' + h + ' (hoster alterno)'); return out; }
      } catch {}
    }
  }
...
/* v348: SEGUNDO ENLACE DE SERIES — mismo espíritu que cqAltBuscar (pelis):
 * si el título existe en el catálogo de Danimados (en memoria, 821 series,
 * numeración TxE idéntica), devolvemos su URL de episodio para que
 * resolverDani la jale por OTRA infraestructura. Coincidencia exacta. */
async function cqSerieAltBuscar(titulo, T, E) {
  const nb = normaBv(titulo);
  if (!nb || nb.length < 4 || !DANI_CAT.size) return null;
  for (const [slug, v] of DANI_CAT) { if (v && normaBv(v.t) === nb) return DANI_BASE + '/episodios/' + slug + '-' + T + 'x' + E + '/'; }
  return null;
}
/* v352: FASE 3 — el último recurso de las PELÍCULAS cq: buscar el título en
 * el catálogo de CUEVANA (en memoria, otro hoster: goodstream) y resolver por
 * allá. resolverCuevanaMov ya cosecha la bóveda cv: → a la segunda, el
 * respaldo la encuentra solo. Negative cache 30 min para no martillar. */
const CQ_CV_FALLO = new Map(); /* normaBv → at del último intento */
async function cqCuevanaFallback(titulo) {
  const nb = normaBv(titulo);
  if (!nb || nb.length < 4 || !CVM_CAT.size) return null;
  const atF = CQ_CV_FALLO.get(nb);
  if (atF && Date.now() - atF < 1800000) return null;
  for (const [slug, v] of CVM_CAT) {
    if (v && normaBv(v.t) === nb) {
      try {
        const out = await resolverCuevanaMov('https://cuevana.mov/pelicula/0/' + slug);
        console.log('[boveda] cq "' + titulo + '" por FASE 3 → Cuevana (' + slug + ')');
        return out;
      } catch (eF) { CQ_CV_FALLO.set(nb, Date.now()); console.log('[boveda] fase 3 cuevana sin video (' + String(eF.message || eF).slice(0, 40) + ')'); return null; }
    }
  }
  CQ_CV_FALLO.set(nb, Date.now()); /* no está en el catálogo de cuevana — 30 min de calma */
  return null;
}
  ...
  if (m) {
    ...
    if (codeB) {
      try {
        ...
        bodegaTag(bkEp, outB && outB.m3u8); /* v355: censo (la serie entera comparte bodega con su archivo) */
    ...
    try { return await resolverVimeos('https://vimeos.net/embed-' + code + '.html', CQ_WEB + '/'); }
      ...
      try { /* v348: segundo enlace de SERIE vía Danimados (cached en altEps) */
        let urlAlt = (bEp && bEp.altEps && bEp.altEps[m[3] + 'x' + m[4]]) || null;
        if (!urlAlt) { const altU = await cqSerieAltBuscar((BOVEDA.get(bkEp) || {}).t || '', +m[3], +m[4]); if (altU) { urlAlt = altU; try { const bvA = BOVEDA.get(bkEp) || {}; bvA.altEps = bvA.altEps || {}; bvA.altEps[m[3] + 'x' + m[4]] = altU; bovedaPon(bkEp, { altEps: bvA.altEps }); console.log('[boveda] SEGUNDO ENLACE de serie: T' + m[3] + 'E' + m[4] + ' → Danimados'); } catch {} } }
        if (urlAlt) { const outD = await resolverDani(urlAlt); console.log('[boveda] cq ' + m[2] + ' T' + m[3] + 'E' + m[4] + ' por SEGUNDO ENLACE (Danimados)'); return outD; }
      } catch (eD) { console.log('[boveda] segundo enlace de serie sin video (' + String(eD.message || eD).slice(0, 40) + ')'); }
  ...
  if (m) {
    ...
    if (bP && bP.code) {
      try {
        ...
        bodegaTag(bkP, outPB && outPB.m3u8); /* v355: censo */
        ...
        const rF1 = await cqCuevanaFallback(bP.t || '').catch(() => null); /* v352: FASE 3 — Cuevana */
        if (rF1) return rF1;
      }
    }
    ...
    try {
      ...
      if (rbP) return rbP;
      const rF2 = await cqCuevanaFallback((d.item && d.item.title) || '').catch(() => null); /* v352: FASE 3 — Cuevana */
      if (rF2) return rF2;
      throw eV;
    }
  }
...
async function sirveElVideo(url, referer) {
  ...
  try {
    ...
    clearTimeout(t);
    okDirecto = r.ok || r.status === 206;
  } catch { clearTimeout(t); }
  if (okDirecto) return true;
  if (CDN_RELAY) { /* v349: la sonda también mira POR EL RELAY — vimeos bloquea datacenters y el canario/auditorías quedaban mudos */
    try { const r2 = await fetchRelay(url, 10000); return r2.ok || r2.status === 206; } catch { return false; }
  }
  return false;
}
  ...
  return mezcla;
    ...
    if (url.pathname === '/api/solo' && req.method === 'GET') {
      ...
      try {
        ...
        try { const urlF = String(target).slice(0, 130); if (!FALLOS_RECIENTES.length || FALLOS_RECIENTES[0].url !== urlF || Date.now() - FALLOS_RECIENTES[0].at > 600000) { FALLOS_RECIENTES.unshift({ url: urlF, fuente: (verifFuenteDe(target) || {}).f || '', titulo: tituloFalloDe(target), err: String(e.message || e).slice(0, 80), at: Date.now() }); if (FALLOS_RECIENTES.length > 30) FALLOS_RECIENTES.length = 30; } } catch {} /* v354: el panel muestra los fallos recientes */
    ...
    if (url.pathname === '/api/set-relay') { /* v236: actualizar CDN relay URL del Mac Mini */
      ...
      { const _mRN = String(newUrl).match(/https?:\/\/[^\s)\]]+/); if (_mRN) newUrl = _mRN[0]; } /* v350: sano el formato (si alguien pega con corchetes/markdown, no rompe el relay) */
      CDN_RELAY = newUrl;
      console.log('[relay] CDN relay actualizado:', CDN_RELAY);
      try { require('fs').writeFileSync(path.join(DATA_DIR, 'relay.txt'), newUrl) /* v350: el set-relay también sanea abajo */; } catch {} /* v316: persistente */
    ...
    if (url.pathname === '/api/vimeos-salud' && req.method === 'GET') { /* v354: el saludómetro del panel */
      const porB = {};
      let clasif = 0;
      for (const b2t of Object.values(CENSO.titulos)) { porB[b2t] = (porB[b2t] || 0) + 1; clasif++; }
      let totalPelis = 0;
      for (const k2 of BOVEDA.keys()) if (k2.startsWith('cq:movie:')) totalPelis++;
      const bodegas = [...VIMEOS_SALUD.entries()].map(([b, r]) => ({ b, ok: r.ok, ko: r.ko, lastOk: r.lastOk, lastKo: r.lastKo, titulos: porB[b] || 0, estado: (Date.now() - Math.max(r.lastOk || 0, r.lastKo || 0) < 3600000) ? (r.lastOk >= r.lastKo ? 'ok' : 'ko') : 'silencio' })).sort((a, b2) => ((b2.ok + b2.ko) || (porB[b2.b] || 0)) - ((a.ok + a.ko) || (porB[a.b] || 0)));
      return json(res, 200, { ok: true, canario: VIMEOS_EST, bodegas, fallos: FALLOS_RECIENTES.slice(0, 15), clasificadas: clasif, totalPelis });
    }
